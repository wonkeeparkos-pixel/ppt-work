...
RLS = [
 ...
 {'t':'table','eyebrow':'진단 · 혈액검사','tag':('감별 패널',''),'title':'RLS 혈액검사 — 무엇을 · 어떻게 해석',
  'foot':'IRLSSG iron 2018 · 원내 척추/관절 패널에 철대사·CBC·CRP·TSH 이미 포함',
  'headers':['검사','목적','해석 요지'],'rows':[
   ['Ferritin ★','RLS 치료 출발점','&lt;30 철결핍 · ≤75 보충 고려'],
   ['TSAT (Fe/TIBC)','철결핍 판정','&lt;20% 철결핍'],
   ['CBC','빈혈 동반','철결핍빈혈 → IV 철분 급여'],
   ['CRP','ferritin 보정','염증 시 ferritin 위양성↑'],
   ['혈당 / HbA1c','당뇨신경병증 감별','≥6.5% 당뇨'],
   ['eGFR · TSH · Mg','신부전·갑상선·NLC','신부전 시 α2δ 감량 등'],
  ],'note':'RLS 진단은 임상(IRLSSG 5기준); 혈액은 ①철분 평가 ②mimic 감별용. 기존 척추/관절 패널로 대부분 커버.'},

 {'t':'split','eyebrow':'병태생리 · 쉽게','tag':('핵심',''),'title':'RLS는 왜 생기나 — 뇌의 철분 부족',
  'foot':'IRLSSG; Allen 2018 (Sleep Med); Garcia-Borreguero 2021 (아데노신)','items':[
   (0,'뇌 특정 부위(흑질) 철분이 부족 — <b>혈액 철분이 정상이어도</b> 뇌엔 부족할 수 있다.',''),
   (0,'철분은 <b>도파민</b>을 만드는 재료 → 부족하면 도파민 ‘진정 신호’가 흔들린다.',''),
   (0,'여기에 아데노신 신호까지 낮아지면 → 밤에 다리가 쉬질 못한다.',''),
   (0,'가족력이 흔하다(유전 경향).',''),
   (-1,'그래서 치료의 출발점은 언제나 ‘철분 확인·보충’.','accent'),
  ],'aside':{'title':'쉽게 말하면','dark':True,'items':[
   (0,'뇌의 <b>진정 브레이크(도파민)</b>가 재료(철분) 부족으로 헐거워진 상태.',''),
   (0,'낮엔 버티다 <b>밤</b>에 브레이크가 더 약해져 증상이 나온다.',''),
   (-1,'철분을 채우면 브레이크 재료가 보충된다.','accent'),
  ]}},
 ...
 {'t':'split','eyebrow':'치료 · 주사','tag':('효과 있음','green'),'title':'주사 ① 정맥 철분(IV FCM) — 근거 최강',
  'foot':'Earley 2024(Sleep, PMID 38625730) · 국내 급여는 철결핍빈혈(ferritin<30 또는 TSAT<20%)에 한함 — RLS 단독 비급여','items':[
 ...
 {'t':'split','eyebrow':'치료 · 경구약제','tag':('조건부','amber'),'title':'경구약제 ③ Dipyridamole · ④ 오피오이드',
  ...
  ],'aside':{'title':'Dipyridamole — 실무','dark':True,'items':[
   (0,'원래 <b>항혈소판·뇌졸중 2차예방</b>약(아데노신↑).',''),
   (0,'국내 단일제는 대부분 <b>수출용</b> · RLS는 <b>비급여</b>(오프라벨).','red'),
   (0,'부작용: 두통(최다)·홍조·저혈압·출혈 경향.','muted'),
   (-1,'국내 RLS 적용은 현실적 제약(약 확보·급여).','accent'),
  ]}},
