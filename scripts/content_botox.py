...
# ── 기전 ──────────────────────────────────────────────────
SVG_MECH_A = f'''<div class="figpanel good"><div class="pt">① 근육을 풀어 통증 고리를 끊는다</div>
<svg viewBox="0 0 320 220" role="img">
 ...
 <text x="150" y="60" font-size="11.5" font-weight="700" fill="{NRVD}">ACh 소포</text>
 <g><line x1="112" y1="95" x2="160" y2="95" stroke="{MUS}" stroke-width="4"/>
    <line x1="128" y1="80" x2="146" y2="110" stroke="{MUS}" stroke-width="5"/>
    <line x1="146" y1="80" x2="128" y2="110" stroke="{MUS}" stroke-width="5"/></g>
 <text x="176" y="99" font-size="12.5" font-weight="800" fill="{MUS}">SNAP-25 절단 → ACh 방출 차단</text>
...
SVG_MECH_B = f'''<div class="figpanel"><div class="pt">② 통증 신호 자체를 줄인다 (직접 진통)</div>
<svg viewBox="0 0 320 220" role="img">
 <text x="160" y="18" text-anchor="middle" font-size="12.5" font-weight="700" fill="{NRVD}">통각신경 종말 · 후근신경절 · 척수</text>
 <path d="M8 58 Q80 58 118 72" fill="none" stroke="#8a5a9a" stroke-width="6" stroke-linecap="round"/>
 <circle cx="126" cy="76" r="14" fill="#E9DDF2" stroke="#8a5a9a" stroke-width="2.5"/>
 <text x="20" y="48" font-size="12" font-weight="700" fill="#6b4480">통각신경(C섬유)</text>
 <text x="150" y="52" font-size="11.5" font-weight="700" fill="#6b4480">substance P</text>
 <text x="150" y="68" font-size="11.5" font-weight="700" fill="#6b4480">CGRP · 글루타메이트</text>
 <g><line x1="150" y1="90" x2="186" y2="90" stroke="{MUS}" stroke-width="4"/>
    <line x1="160" y1="80" x2="176" y2="100" stroke="{MUS}" stroke-width="5"/>
 ...
 <path class="a-flow" d="M140 148 H222" fill="none" stroke="{NRV}" stroke-width="3" marker-end="url(#ar)"/>
 <defs><marker id="ar" markerWidth="8" markerHeight="8" refX="6" refY="4" orient="auto">
   <path d="M0 0 L8 4 L0 8 z" fill="{NRV}"/></marker></defs>
 <text x="16" y="152" font-size="12" font-weight="800" fill="{NRVD}">역행성 축삭수송</text>
 <text x="16" y="186" font-size="12" font-weight="700" fill="{MUTED}">신경인성 염증 ↓</text>
</svg>
<div class="pl">그래서 <b>근육이 아닌 관절강·근막·신경 주위</b>에 놓아도 통증이 준다</div></div>'''
...
{'t': 'bullets', 'eyebrow': '들어가며', 'tag': '이 발표의 틀',
 ...
 'items': [
   ...
   (0, '이 발표는 부위별로 <b>① 근거 → ② 주사기법 → ③ 용량 → ④ 부작용 → ⑤ 회피법</b> 순서로 정리한다.', 'accent'),
...
# 03 ─────────────────────────────────────────────────────
{'t': 'figure', 'eyebrow': '기전 · 그림으로', 'tag': '두 갈래',
 'title': '왜 근육에 놓았는데 통증이 줄어드나',
 'svg': SVG_MECH_A + SVG_MECH_B,
 'caption': '보툴리눔의 진통은 <b>근이완의 부산물이 아니다</b> — 통각 종말에서 신경펩타이드 방출을 직접 막고, 역행성 수송으로 중추 감작까지 낮춘다',
 'foot': 'Matak & Lacković 2014; Mechanisms of BoNT-A action on pain (PMC6723487)'},
...
{'t': 'bullets', 'eyebrow': '기전 · 임상적 함의', 'tag': '시간표',
 ...
 'items': [
   ...
   (0, '<b>진통이 근력약화보다 오래 간다.</b> 근력은 6–8주에 회복되는데 통증 완화는 12주 이상 유지되는 경우가 흔하다 → 감작이 리셋됐다는 방증.', 'accent'),
   (0, '<b>1회로 끝내지 않는다.</b> 근막통·건병증은 근본 부하(자세·작업·근력)를 같이 고치지 않으면 3–4개월 뒤 그대로 돌아온다.', ''),
   (0, '<b>재주사는 최소 12주 간격.</b> 3개월 미만 간격·고누적용량·3주 내 보충주사(booster)는 중화항체를 유도해 이차 무반응을 만든다.', 'red'),
   (-1, '＊효과 판정은 <b>4주 시점</b>에 한다. 2주에 “효과 없다”고 판단하면 너무 이르다.', 'accent'),
 ],
 'foot': '제품별 첨부문서 · 면역원성 리뷰 (Toxins 2019; PMC8795657)'},
...
{'t': 'table', 'eyebrow': '총괄', 'tag': ('근거 지도', 'ink'), 'dense': True,
 ...
 'rows': [
   ['외상후 · 삼차 · 대상포진후 신경통', '<b>A</b>', 'AAN 기준 effective (Jabbari 2018)'],
   ['<b>족저근막염</b>', '<b>B</b>', 'RCT 7편 305명 메타 (PLOS One 2024)'],
   ['<b>이상근증후군</b>', '<b>B</b>', 'SR 7편 152명 · Fishman 2002'],
   ['<b>만성 요통</b>', '<b>B</b>', 'Foster 2001 · SR/MA (Eur J Pain 2025)'],
   ['<b>당뇨병성 신경병증 · 근경련</b>', '<b>B</b>', 'Restivo 2018 RCT (n=50)'],
   ['인공슬관절 치환 후 난치성 통증 · 무릎 OA', '<b>B</b>', 'Singh 2010 · IA 메타 6편 348명'],
   ['<b>외측상과염</b>', '<b>C</b> (상충)', '위약엔 우월, 스테로이드와 대등 · 손가락 약화'],
   ['<b>승모근 · 목어깨 근막통증</b>', '<b>C</b> (제한적)', 'MA 7편: 통계적 유의, <b>임상적 유의성 미달</b>'],
   ['흉곽출구증후군 (사각근)', '<b>U</b>', '증례군 긍정 · RCT 음성 (Finlayson 2011)'],
 ],
 'hlrows': [7, 8],
 'note': 'A=효과 있음 · B=아마 효과 있음 · C=효과 가능성 · U=근거 불충분(AAN). <b>빨간 칸 = 문의는 가장 많고 근거는 가장 약한 두 부위.</b>',
...
# 29 ────────────────────────────────────────────────────
{'t': 'bullets', 'eyebrow': '신경병증 통증', 'tag': ('Level A', 'green'),
 'title': '가장 근거가 강한 영역은 사실 “신경통”이다',
 'items': [
   (0, '<b>Level A(효과 있음):</b> 외상후신경통 · 삼차신경통 · 대상포진후신경통 — 정형외과에서 만나는 <b>수술 후 절개부 신경통</b>이 여기 포함된다.', 'green'),
   (0, '<b>주사법이 다르다:</b> 근육이 아니라 <b>통증 부위 피내·피하에 격자(grid) 형태로 소량 분할</b>(예: 2.5–5 U씩 1–2 cm 간격).', 'accent'),
   (0, '<b>절단 후 통증:</b> 잔단통(RLP)은 보툴리눔·스테로이드 모두 6개월 개선. <b>환상통(PLP)은 변화 없었다</b> — 표적이 말초라는 증거.', ''),
   (0, '<b>신경종 주위 주사:</b> 최근 다기관 연구에서 1개월 시점 환상통 <b>4점 감소 vs 대조 1점</b>, 단 3개월엔 종합치료군이 우수.', ''),
   (0, '<b>복합부위통증증후군(CRPS)·수근관증후군:</b> 근거 불충분 — 특히 수근관은 부정적 결과가 많다.', 'red'),
 ],
...
{'t': 'key', 'eyebrow': 'Clinical Algorithm',
 ...
 'msgs': [
   ...
   ('3 · 부위별 기대치', '<b>족저근막염·이상근·요통·경련·무릎</b>은 근거를 갖고 권한다. <b>승모근 근막통·테니스엘보</b>는 “선택지 중 하나”로 설명한다.'),
