...
LSB = [
 ...
 {'t':'photo','eyebrow':'해부','title':'요추 교감신경간 — 어디를 겨냥하나',
  'caption':'교감신경절은 척추체 <b>전외측 모서리</b>에 좌우로 하나씩, <b>대동맥에 바짝 붙어</b> 종주한다 — 표적은 <b>L2–L3</b>. '
            '이 인접성이 <b>혈관내 주입</b>을 핵심 위험으로 만든다. '
            '· Complete Anatomy 3D 모형에 라벨(교육용 인용)',
  'img':FIG('ca_target', IMG('lsb_anatomy_merged.png','요추 교감신경간 해부', jpeg=True, q=88),
            'L3 축상면 — 교감신경절과 주변 구조')},
 # ═══════════ 감별 — 이 환자가 맞나 ═══════════
 # ═══════════ 술기 — 어떻게 하나 ═══════════
...
# ---------- 실제 해부(Complete Anatomy, L3 body level) 회피 슬라이드 ----------
# assets/ca_avoid.{webp,png,jpg} 가 들어오면 자동으로 덱에 추가된다.
# 없을 때는 21쪽 오리지널 도해와 그림이 겹치므로 아예 넣지 않는다.
if any(os.path.exists(os.path.join(ASSETS, "ca_avoid." + e))
       for e in ("webp", "png", "jpg")):
    LSB.append({'t':'photo','eyebrow':'안전 · 실제 해부',
      'title':'표적에서 벗어나면 무엇을 만나나 — L3 실제 단면',
      'caption':'벗어나는 방향마다 만나는 것이 다르다 — <b>결국 한 문장이다. 뼈를 놓치지 말 것.</b> '
                '· Complete Anatomy 3D 모형에 라벨(교육용 인용)',
      'img':FIG('ca_avoid', '', '실제 해부 — L3 단면에서 본 신장·대혈관과 안전 경로')})
...
_ORDER = ['요추교감신경차단', '전체 흐름',
          ...
          'L2·L3 시술', '혈관·신장 천자', '조영제 확산 읽기',
          # ── 합병증 ──
          '합병증 & 회피법', '표적에서 벗어나면', '생식대퇴신경통', '혈관·LAST·신경 손상', '합병증 대비',
          # ── 확인과 효과 ──
          '확인과 효과 — 진단적', '확인의 함정', '효과 ① 관류', '효과 ② CRPS', '근거수준',
          # ── 마무리 ──
