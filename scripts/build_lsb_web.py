...
if any(os.path.exists(os.path.join(ASSETS, "ca_avoid." + e))
    ...
    LSB.append({'t':'photo','eyebrow':'안전 · 실제 해부',
      'title':'실제 해부로 본 회피 — 신장과 대혈관은 어디에 있나 (L3 body level)',
      'caption':'L3 척추체 높이의 실제 단면 — <b>신장</b>은 후외측, <b>대동맥·IVC</b>는 척추체 바로 앞. '
                '표적인 교감신경간은 그 사이 <b>척추체 전외측 모서리</b>에 있다. '
                '<b>뼈에 붙여 들어가고 척추체 전연을 넘기지 않으면</b> 두 구조를 모두 지나치지 않는다. '
                '· Complete Anatomy 캡처(교육용 인용)',
...
_ORDER = ['요추교감신경차단', '전체 흐름',
          ...
          # ── 합병증 ──
          '합병증 & 회피법', '실제 해부로 본 회피', '생식대퇴신경통', '혈관·LAST·신경 손상', '합병증 대비',
          # ── 확인과 효과 ──
