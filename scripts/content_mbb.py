...
# ══════════════════════════════════════════════ 07 효과와 논쟁
add(
    t='sec', no='07', title='효과는 어디까지 증명됐나',
    desc='진단이 맞아도 시술이 실패할 수 있다. 그리고 그 실패의 상당 부분은 '
         '환자 선정이 아니라 <b>바늘이 신경을 못 잡아서</b> 생긴다.',
)

add(
    t='split', eyebrow='RFA · 기술적 실패',
    title='최적화된 표적에서도 신경을 <b>완전히</b> 잡는 것은 42%뿐',
    kick='효과 논쟁을 “선정 문제”로만 읽으면 절반을 놓친다.',
    items=[
        (0, '사체에서 투시 유도로 L3·L4·L5 내측지에 캐뉼라 12개를 놓고 병변을 시뮬레이션했다. '
            '<b>완전 포착 5/12(41.7%)</b> · 부분 포착 4/12(33.3%) · <b>포착 실패 3/12(25%)</b>.', 'hr_'),
        (0, '<b>전극은 신경과 평행</b>해야 한다 — 수직으로 놓으면 “신경이 응고를 피하거나 '
            '부분적으로만 응고될 수 있다”(Lau 2004). 이것이 RFA와 진단 차단의 결정적 차이다.', 'hi'),
        (0, '실제 임상에서는 <b>미측 각도가 부족하다</b>. 사체 최적치 대비 환자 영상의 평균 각도가 '
            'L3 <b>18.9°</b> · L4 <b>15.1°</b> · L5 <b>11.7°</b>만큼 얕았다.', 'hr_'),
        (0, '해부가 표적을 좁힌다 — SAP 외측 경부 중간에서 후지는 이미 <b>96.7%가 분지</b>해 있고, '
            '골막 접촉 길이는 L1 57% → L5 82%로 레벨마다 다르다.'),
        (0, 'MAL이 <b>골화</b>되면 뼈 지붕이 생겨 캐뉼라가 의도한 골막면에서 밀려난다.', 'hr_'),
    ],
    aside=dict(title='병변 포착 (사체, n=12)', tone='crit', stat=[
        ('41.7%', '완전 포착 — 본간 또는 전 분지 포함'),
        ('33.3%', '부분 포착 — 원위 분지 일부 이탈', 'r'),
        ('25.0%', '포착 실패', 'r'),
    ], note='표적을 <b>복측</b>으로, 전극을 <b>평행</b>하게, 각도를 <b>더 미측</b>으로 — '
            '세 가지가 모두 맞아야 한다.'),
    foot='Tran J, Conger A, Lightfoot K, McCormick ZL, Loh E. Interv Pain Med 2025 (PMC12051118) · '
         'Lau P et al. Pain Med 2004;5(3):289–298 (PMID 15367308) · Interv Pain Med 2024 (PMC11536316) · '
         'Tran J et al. Interv Pain Med 2024;3(2):100414',
)
...
add(
    t='split', eyebrow='PROGNOSIS · 개별 매핑',
    title='집단 지도 대신 <b>그 환자의 지도</b>를 그린다',
    kick='연관통 지도로 레벨을 못 고른다면, 시술 중에 환자 본인에게 확인하면 된다.',
    items=[
        (0, '진단 차단을 마친 15명에서 RF 캐뉼라를 놓은 뒤 50 Hz로 감각 역치를 구하고 '
            '<b>역치 이상 자극</b>을 주어, 환자가 느낀 부위를 시술 전 통증 그림과 대조했다.'),
        (0, '전원이 통증·이상감각을 느꼈고, <b>15명 중 14명</b>에서 자극 부위가 원래 통증 영역을 '
            '<b>완전히 덮었다</b>.', 'hi'),
        (0, '저자 결론 — 후관절통은 역치 이상 감각자극으로 <b>매핑할 수 있으며</b>, '
            'RF 신경차단술에 객관성을 도입할 잠재력이 있다.'),
        (-1, ''),
        (0, '이 발표 전체의 실무적 귀결 — <b>집단 연관통 지도는 레벨 선택 도구가 아니다.</b> '
            '레벨은 차단으로 고르고, 시술 중에는 <b>개별 감각 매핑</b>으로 확인한다.', 'hi'),
    ],
    aside=dict(title='아직 열려 있는 것', tone='crit', items=[
        (0, '차단을 <b>몇 번</b> 할 것인가 — 0회·1회·2회 진영이 모두 무작위 근거를 갖고 있다', 'hr_'),
        (0, '역치 <b>50% vs 80%</b> — 학회 간 불일치', 'hr_'),
        (0, '어떤 약제로, 언제 결과를 평가할 것인가'),
        (-1, ''),
        (0, 'ASRA 2024 종설의 제목이 그대로 현 상황이다 — '
            '<b>“None, Once, or Twice?”</b>', 'hj'),
    ]),
    note='2년 추적에서 80% 기준 responder의 89.5%가 여전히 후관절성이었고 50% 기준군은 51%였다는 '
         '보고가 있으나, 이 수치는 <b>이차 요약에서 얻은 것</b>으로 원문 확인이 필요하다.',
    foot='Sensory mapping of lumbar facet joint pain: a feasibility study. 2021 (PMID 33633853) · '
         'Assavanop S, Bhatia A. ASRA Pain Medicine News 2024 · 국제 설문 PMID 39029928',
)

# ══════════════════════════════════════════════ 08 합병증·안전성
add(
    t='sec', no='08', title='안전성 — 무엇이 실제로 측정되었나',
    desc='이 시술의 위해는 대부분 “합병증”이 아니라 <b>주입액이 가면 안 될 곳에 가는 것</b>으로 나타난다. '
         '그것은 실측되어 있다.',
)

add(
    t='table', eyebrow='SAFETY · 실측된 위해',
    title='측정된 것과, 아직 측정하지 못한 것',
    kick='표적을 벗어난 확산이 안전성 문제이자 동시에 진단 오류의 원인이다.',
    dense=True,
    headers=['항목', '수치', '출처', '임상적 의미'],
    rows=[
        ['혈관내 주입 (MBB)', '<b>6.1%</b>/신경 (실시간 투시)', 'Lee 2008 · 1433건',
         '약이 씻겨나가 <b>위음성</b>'],
        ['혈관내 주입 (DSA 기준)', '<b>≈19%</b> vs 실시간 투시 11%', 'Pain Med 2016 · 344건',
         '실시간 투시도 과소검출'],
        ['흡인 검사 민감도', '<b>34.1%</b> (정지영상 59.1%)', 'Lee 2008',
         '흡인 단독은 부적합'],
        ['0.5 mL의 경막외·척추간공 확산', '<b>16%</b>', 'Dreyfuss 1997 · CT',
         '용량이 곧 특이도'],
        ['관절강내 주사의 경막외 확산', '환자 <b>64.6%</b> · 시술 <b>49.5%</b>', 'Yoo 2020 · 192건',
         '진단 특이도 상실'],
        ['방사선량 (AP vs 사위)', '<b>66</b> vs <b>109</b> mGy', '2024 · 180명',
         '정확도 동일, 피폭 절반'],
        ['감염·출혈·척수마취·LAST', '<b>수치 미확보</b>', '—',
         '<b>이번 고찰에서 다루지 못함</b>'],
    ],
    hlrows={4: 'hlj', 6: 'hl'},
    note='마지막 행이 이 발표의 가장 큰 공백이다. <b>ASRA/ESRA 2018 항응고 위험 계층화</b>와 '
         '감염·출혈 발생률은 별도 확인이 필요하다 — 시술 전 항응고 관리 방침은 이 자료로 대신할 수 없다.',
    notec='warn',
    foot='Lee CJ et al. Anesth Analg 2008 (PMID 18349205) · Pain Medicine 2016;17(6):1031 · '
         'Dreyfuss P et al. Spine 1997 (PMID 9127924) · Yoo BR et al. Acta Radiol 2020 (PMID 31510763) · '
         'PMC11433151 (2024)',
)

add(
    t='split', eyebrow='SAFETY · 다열근',
    title='다열근 탈신경은 부작용인가, 불가피한 대가인가',
    kick='내측지 하나를 태우면 그 분절의 다열근도 함께 신경을 잃는다. 피할 방법이 없다.',
    items=[
        (0, '다열근은 <b>단분절 지배</b>를 받는다 — 각 내측지는 자기 번호의 극돌기에서 기시하는 '
            '다발만 지배한다. 따라서 유효한 병변은 <b>반드시</b> 해당 분절 다열근을 함께 탈신경시킨다.', 'hi'),
        (0, '내측지는 관절만이 아니라 <b>다열근·극간근·내측횡돌기간근·극간인대·골막</b>도 지배한다. '
            '차단이 관절 통증만 지우는 것이 아니라는 뜻이기도 하다.'),
        (0, '<b>Dreyfuss 2009</b> — 편측 RFN 성공 5명, 17–26개월 MRI를 방사선의사 3명이 맹검 판독. '
            'EMG상 전원 탈신경이 확인되었지만, MRI에서 <b>어느 쪽·어느 분절인지 가려내지 못했다</b>. '
            '미만성 위축은 보였다.'),
        (0, '<b>반대 근거</b> — 2024년 편측 RFN 연구는 시술측의 지방 위축 증가를 보고했고(PMID 38635483), '
            '전단탄성률 감소를 보고한 연구도 있다(PMID 35832607).', 'hr_'),
        (0, '2025년 종설의 결론은 <b>근거가 빈약하고 이질적</b>이라는 것이다 — 대부분 30명 미만.'),
    ],
    aside=dict(title='실무 함의', tone='nerve', items=[
        (0, '다열근 탈신경은 <b>실패의 신호가 아니라 성공의 부산물</b>이다 — 병변이 신경을 잡았다는 뜻.'),
        (-1, ''),
        (0, '그래서 RFA 후 <b>분절 안정화 운동</b>을 함께 처방하는 근거가 된다.', 'hi'),
        (-1, ''),
        (0, 'MINT가 RFA에 운동요법을 병행시킨 것도 이 맥락에서 읽어야 한다 — '
            '다만 그 병행이 신호를 희석했다는 반론이 있다.', 'hj'),
    ]),
    foot='Macintosh JE et al. Clin Biomech 1986 · Bogduk N. Spine 1983 (PMID 6226119) · '
         'Dreyfuss P et al. PM R 2009;1(8):719–722 (PMID 19695523) · PMID 38635483 · PMID 35832607',
)
...
add(
    ...
    items=[
        ...
        (0, '<b>관절강내 고전 무작위 시험 5편</b>(Carette·Lilius·Nash·Marks·Cochrane)은 '
            '서지사항만 확인했다 — 인용 전 원문 필수.', 'hr_'),
        (0, '<b>감염·출혈·척수마취·LAST 발생률</b>과 <b>ASRA/ESRA 2018 항응고 위험 계층화</b>는 '
            '끝내 확보하지 못했다. 시술 전 항응고 관리 방침은 이 자료로 대신할 수 없다.', 'hr_'),
        (0, '“효과·논쟁”·“안전성” 섹션은 <b>다른 주제에서 확보된 검증 자료</b>로 재구성했다 — '
            '해당 주제 전용 문헌검색은 수행하지 못했다(세션 한도 소진).', 'hr_'),
...
# ══════════════════════════════════════════════ 슬라이드 키
# 덱을 둘로 나눠 조립하기 위한 안정적인 식별자. SLIDES 순서와 1:1로 대응한다.
KEYS = [
    'title',
    'sec_anat', 'anat_posterior', 'anat_mapping', 'anat_naming', 'anat_mal', 'anat_l5',
    'sec_ref', 'ref_zones', 'ref_table', 'ref_controversy', 'ref_mechanism', 'ref_cervical',
    'ref_predictors',
    'sec_dx', 'dx_prevalence', 'dx_falsepos', 'dx_threshold', 'dx_algorithm', 'dx_facts',
    'sec_tech', 'tech_target', 'tech_carm', 'tech_proc1', 'tech_proc2', 'tech_depth',
    'tech_volume', 'tech_vascular', 'tech_us', 'tech_us_fig',
    'sec_ia', 'ia_orientation', 'ia_technique', 'ia_specificity', 'ia_evidence',
    'sec_ind', 'ind_compare', 'ind_selection',
    'sec_out', 'out_rfa_fail', 'out_mint', 'out_prognosis',
    'sec_safe', 'safe_table', 'safe_multifidus',
    'summary', 'limitations', 'ref1', 'ref2', 'ref3', 'ref4', 'ref5',
]
assert len(KEYS) == len(SLIDES), f"KEYS {len(KEYS)} != SLIDES {len(SLIDES)}"
BY_KEY = {}
for _k, _s in zip(KEYS, SLIDES):
    _s['key'] = _k
    BY_KEY[_k] = _s
